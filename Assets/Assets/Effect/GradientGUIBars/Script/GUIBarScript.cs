...
public class GUIBarScript : MonoBehaviour {
	...
	void OnGUI()
	{
		...
		if (Background != null)
		{
			GUI.DrawTexture(new Rect(Position.x,Position.y-100,Background.width * ScaleSize,Background.height * ScaleSize),Background);
		}

		GUI.DrawTexture(new Rect(Position.x,Position.y-100,ValueBar.width * ScaleSize,ValueBar.height * ScaleSize),ValueBar);

		//if you don't have a foreground texture i won't draw it
		if (Foreground != null)
		{
			GUI.DrawTexture(new Rect(Position.x,Position.y-100,Foreground.width * ScaleSize,Foreground.height * ScaleSize),Foreground);
		}
		...
		if (DisplayText)
		{
			...
			GUI.Label(new Rect(Position.x + TextOffset.x,Position.y-100 + TextOffset.y,ValueBar.width * ScaleSize,ValueBar.height * ScaleSize),TextString,LabelStyle);
